...
export default function Order() {
   ...
   const handleNavigateToPayment = () => {
    ...
    navigation.navigate('Payment', {
        ...
        order: route.params.order_id, // Correção aplicada
        total: total,
    });
};
        ...
        <SafeAreaView style={styles.containerLayout}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerRight}>
                    <TouchableOpacity style={{ marginRight: 15 }}><Ionicons name="cart-outline" size={24} color="#333" /></TouchableOpacity>
                                ...
                                <View style={styles.productsListView}>
                                    {cat.products.map((prod: Product) => (
                                        <View key={prod.id} style={styles.productContainer}>
                                            {prod.banner ? (
                                                <Image source={{ uri: prod.banner }} style={styles.productImage} resizeMode="cover" />
                                            ) : (
            ...
            {hasOrderItems && (
                <>
                    <View style={styles.summaryContainer}>
                        <FlatList data={orderSummary} keyExtractor={(item) => item.id + item.amount} horizontal showsHorizontalScrollIndicator={false} renderItem={({ item }) => (<View style={styles.summaryItem}><Text style={styles.summaryText}>{item.name} x {item.amount}</Text></View>)} />
                    </View>
...
const styles = StyleSheet.create({
    ...
    totalText: { color: "#FFF", fontSize: 25, fontWeight: "bold" },
});

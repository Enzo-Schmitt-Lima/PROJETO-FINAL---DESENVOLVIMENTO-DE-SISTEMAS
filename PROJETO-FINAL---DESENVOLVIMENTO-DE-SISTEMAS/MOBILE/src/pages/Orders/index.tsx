...
type OrdersRouteProp=RouteProp<AppStackParamsList,'Orders'>;
interface Order{
id:string;
...
// Desestruturação segura para pegar os parâmetros do pedido atual (se existirem)
...
console.log('Erro ao carregar pedidos:',err,(err as any)?.response?.data);
...
return()=>{
try{
s.off('order:update',onOrderUpdate);
s.off('order:create',onOrderCreate);
}catch{}
};
},[isGuest,user,navigation]);
const calculateTotal=(items:Order['items'])=>
items.reduce((total,item)=>total+item.amount*parseFloat(item.product.price),0);
const getDesc=(items:Order['items'])=>
items.map(item=>`${item.product.name} x${item.amount}`).join(' + ');
...
onPress={()=>{
// ➡️ CORREÇÃO: Navega para a tela "Order" com os parâmetros do pedido atual.
navigation.navigate("Order",{order_id:order_id,number:number});
}}
>
...
onPress={()=>navigation.navigate('OrderStatus',{order_id:order.id})}
>
...
backgroundColor:statusColor(order.status),marginRight:8
}}
/>
<Text style={styles.orderText}>{getDesc(order.items)}</Text>
</View>
<Text style={[styles.orderSub,{fontWeight:'bold'}]}>
{statusText(order)}
</Text>
</View>
...
<Text style={[styles.orderSub,{marginTop:6}]}>
Pagamento: {order.pagamento[0].statusText||order.pagamento[0].metodo||'—'}
</Text>
)}
</TouchableOpacity>
))
...
cardContainer:{flex:1,backgroundColor:'#F5F5F5',borderRadius:30,padding:20,width:'92%',maxWidth:420,alignItems:'stretch',position:'relative'},
backButton:{position:'absolute',left:18,padding:10,zIndex:10},
refreshButton:{position:'absolute',right:10,padding:8},
backText:{color:'#911F09',fontSize:16,fontWeight:'bold'},
logo:{color:'#B72F14',fontWeight:'bold',fontSize:18,marginBottom:8},
title:{fontSize:22,fontWeight:'bold',color:'#1A3A6B',marginBottom:18,marginTop:10,textAlign:'center'},
searchBar:{backgroundColor:'#EEE',borderRadius:8,padding:6,marginBottom:12},
searchInput:{fontSize:16,color:'#1A3A6B',padding:4},
orderBox:{backgroundColor:'#B72F14',borderRadius:8,padding:14,marginBottom:12},
orderText:{color:'#FFF',fontWeight:'bold',fontSize:16,width:'72%'},

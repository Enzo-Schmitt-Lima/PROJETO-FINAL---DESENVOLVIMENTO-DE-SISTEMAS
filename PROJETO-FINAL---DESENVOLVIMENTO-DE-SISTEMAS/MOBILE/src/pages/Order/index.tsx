...
export default function Order() {
    ...
    const loadOrderItems = useCallback(async () => {
        ...
        try {
            ...
            if (isPaid) {
                // Limpa o carrinho se o pedido já foi pago
    ...
    const handleCancelOrder = async () => {
        try {
            // Se a API suportasse um endpoint `/order/clear` ou similar, seria mais eficiente.
            const response = await api.get(`/order/detail?order_id=${route.params.order_id}`);
            const items = (response.data?.items || []) as OrderItem[];

            // Promessas para remover todos os itens em paralelo
            await Promise.all(
    ...
    const filteredCategories = useMemo(() => {
        ...
        return categories
            .map(category => {
                ...
                // Inclui a categoria se o nome dela corresponder à busca, mesmo sem produtos filtrados
                            ...
                            {(openCategory === cat.id || !!searchText) && (
                                <View style={styles.productsListView}>
                                    {cat.products.map((prod: Product) => {
                                        ...
                                        const authRequired = prod.name.toLowerCase().includes('calabresa'); // Idealmente, isso viria da API
                                        return (
            ...
            <HamburgerMenu
                onNavigate={(route: string) => {
                    if (route === 'Home') {
                        ...
                        // Idealmente, deveria navegar para uma tela de seleção de mesa ou recarregar a atual
...
const styles = StyleSheet.create({
    ...
    productContainer: {
        width: "48%", // Ajustado para melhor espaçamento
    ...
    productImageLarge: {
        ...
        height: 100,
        borderRadius: 6,
        marginBottom: 8, // Espaçamento abaixo da imagem
    },
    productInfo: {
        flex: 1,
    ...
    productDesc: {
        ...
        lineHeight: 14,
        textAlign: "center",
        minHeight: 28, // Garante altura mínima para 2 linhas
        marginBottom: 4,
    },
    productPrice: {
        color: "#101026",
